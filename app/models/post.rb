class Post < ActiveRecord::Base
  ...
  def generate_activity_item_on_create
    ActivityItemWorker.new("create", self.id, "post", title, user_id).perform
  end

  def generate_activity_item_on_update
    ActivityItemWorker.new("update", self.id, "post", title, user_id).perform
  end

  def generate_activity_item_on_destroy
    ActivityItemWorker.new("delete", self.id, "post", title, user_id).perform
  end
end
